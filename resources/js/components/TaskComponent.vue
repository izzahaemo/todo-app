<template>
                        ...
                        <table class="table text-center">
                            ...
                            <tbody>
                                <tr v-for="(task, index) in tasks" :key="index">
                                    ...
                                    <td>
                                        <a
                                            :href="`${url}${task.task_file}`"
                                            target="_blank"
                                            v-show="task.task_file"
                                        >
                                            <img
                                                :src="`${url}${task.task_file}`"
                                                alt="task_file"
                                                width="100"
                                            />
                                        </a>
                                        <p v-show="!task.task_file">...</p>
                                    </td>
                                    <td>
                                        <a
                                            class="btn btn-dark"
                                            :href="url2 + task.other_file"
                                            v-show="task.other_file"
                                            target="_blank"
                                        >
                                            Download
                                        </a>
                                        <p v-show="!task.other_file">...</p>
                                    </td>
                                    <td>
                                        <span
                                            :class="`badge ${
    ...
    <div
        ...
        id="subTaskModal"
        tabindex="-1"
        aria-labelledby="subTaskModalLabel"
        aria-hidden="true"
    >
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body">
                                ...
                                >
                                    ...
                                    <td>
                                        ...
                                        <button
                                            ...
                                            v-if="sub_task.status == 0"
                                            @click="markAsComplete(sub_task)"
                                        >
                                            Mark As Complete
                                        </button>
                                        <button
                                            class="btn btn-warning btn-sm mx-1"
                                            v-if="sub_task.status == 1"
                                            @click="markAsIncomplete(sub_task)"
                                        >
                                            Mark As Incomplete
                                        </button>
                                    </td>
                    ...
                    <div
                        class="row mt-3 mb-3"
                        v-for="(subTaskData, index) in subTaskDatas"
                        :key="index"
                    >
                        <div class="col-md-2">
                            <div class="form-group">
                                ...
                                <span
                                    class="text-danger"
                                    v-show="subTaskErrors[index].title"
                                    >title is required</span
                                >
                            </div>
                        </div>
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input
                                    type="date"
                                    class="form-control"
                                    v-model="subTaskData.start_date"
                                />
                                <span
                                    class="text-danger"
                                    v-show="subTaskErrors[index].start_date"
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input
                                    type="date"
                                    class="form-control"
                                    v-model="subTaskData.end_date"
                                />
                                <span
                                    class="text-danger"
                                    v-show="subTaskErrors[index].end_date"
                                    >end_date is required</span
                                >
                            </div>
                        </div>
                        <div class="col-md-1" v-if="!subEditMode">
                            <div class="form-group">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <button
                                            @click="removeSubTaskData(index)"
                                            v-if="
                                                subTaskDatas.length > 1 &&
                                                index > 0
                                            "
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <button
                                class="btn btn-info btn-sm float-end mt-2"
                                @click="
                                    !subEditMode
                                        ? storeSubTask()
                                        : updateSubTask()
    ...
    methods: {
        ...
        storeTask() {
            ...
            if (
                ...
                axios
                    .post(window.url + "api/storeTask", formData, config)
                    .then((response) => {
                        this.getTasks();
                        console.log(response.data);
                    })
        ...
        storeSubTask() {
            for (let i = 0; i < this.subTaskDatas.length; i++) {
                this.subTaskDatas[i].title == ""
                    ? (this.subTaskErrors[i].title = true)
                    : (this.subTaskErrors[i].title = false);
                this.subTaskDatas[i].start_date == ""
                    ? (this.subTaskErrors[i].start_date = true)
                    : (this.subTaskErrors[i].start_date = false);
                this.subTaskDatas[i].end_date == ""
                    ? (this.subTaskErrors[i].end_date = true)
                    : (this.subTaskErrors[i].end_date = false);
            }
            ...
            if (result) {
                ...
                setTimeout(function () {
                    axios
                        .post(window.url + "api/storeSubTask", t.subTaskDatas)
                        .then((response) => {
                            console.log(response.data);
                            t.getTasks();
        ...
        deleteSubTask(sub_task) {
            if (confirm("Are you sure you want to delete this sub task?")) {
                ...
                setTimeout(function () {
                    axios
                        .post(
                            window.url + "api/deleteSubTask/" + sub_task.id,
                            t.subTaskDatas
                        )
                        .then((response) => {
                            t.getTasks();
        ...
        markAsComplete(sub_task) {
            if (
                confirm(
                    "Are you sure you want to mark this sub task as complete?"
                )
            ) {
                window.emitter.emit("loaderStatus", true);
                const t = this;
                setTimeout(function () {
                    axios
                        .post(
                            window.url + "api/markAsComplete/" + sub_task.id,
                            t.subTaskDatas
                        )
                        .then((response) => {
                            t.getTasks();
        ...
        markAsIncomplete(sub_task) {
            if (
                confirm(
                    "Are you sure you want to mark this sub task as incomplete?"
                )
            ) {
                window.emitter.emit("loaderStatus", true);
                const t = this;
                setTimeout(function () {
                    axios
                        .post(
                            window.url + "api/markAsIncomplete/" + sub_task.id,
                            t.subTaskDatas
                        )
                        .then((response) => {
                            t.getTasks();
